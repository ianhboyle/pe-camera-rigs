...
2. **Configure Camera Data:**
   ...
   - **Lens → Focal Length:** `5.2` mm (Canon RF spec)
...
```
Left Eye Image ──→ Scale Left ──→ Translate Left ──→┐
                                                     ├─→ Alpha Over ──→ File Output
Right Eye Image ──→ Scale Right ──→ Translate Right ──→┘                    ↓
                                                                      Composite (optional)
```
...
- Check Translate X values: left=-1920, right=+1920
